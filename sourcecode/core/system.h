...
namespace Nexus
{
	...
	public:
		...
		inline const std::string& getRootDir(void) { return _mStrRootDir; }

		// Get OS's logged in user name
		inline const std::string& getUsername(void) { return _mStrUserName; }
		...
		inline const std::string& getUserAppDataDir(void) { return _mStrAppDataDir; }
		...
		inline const std::string& getUserMyDocumentsDir(void) { return _mStrMyDocumentsDir; }

		// Get the directory path to the user's desktop
		// In XP this is C:\Documents and Settings\USER NAME\Desktop\ 
		inline const std::string& getUserDesktopDir(void) { return _mStrDesktopDir; }
		...
		inline const std::string& getCPUvendorName(void) { return _mStrCPUVendor; }

		// Returns desktop's initial width before application started.
		inline unsigned int getDesktopWidth(void) { return _miDesktopDim[0]; }

		// Returns desktop's initial height before application started.
		inline unsigned int getDesktopHeight(void) { return _miDesktopDim[1]; }

		// Returns desktop's initial refresh rate in Hz
		inline unsigned int getDesktopHz(void) { return _miDesktopHz; }
		...
		inline float getDesktopAspectRatio(void) { return _mfDesktopRatio; }
	...
	private:
		std::string _mStrRootDir;					// App's root directory
		std::string _mStrUserName;				// Logged in user's name
		//	std::string _mStrOSName;					// OS version name
		std::string _mStrCPUVendor;				// CPU Vendor name
		std::string _mStrAppDataDir;				// Stores directory path to user's app data directory (In XP this is C:\Documents and settings\USER NAME\Application data\)
		std::string _mStrMyDocumentsDir;			// Stores directory path to user's My Documents directory (In XP this is C:\Documents and Settings\USER NAME\My Documents\)
		std::string _mStrDesktopDir;				// Stores directory path to user's Desktop directory (In XP this is C:\Documents and Settings\USER NAME\Desktop\)
		//	bool _mbSSEsupport;					// SSE supported by hardware and OS?
		unsigned int _miDesktopDim[2];		// Desktop window's width and height
		bool _mbCPUFreqCalculated;			// Has the CPU frequency already been calculated?
		float _mfCPUFreqHz;					// Holds CPU frequency in Hz
		unsigned int _miDesktopHz;			// Desktop window's initial vsync rate of the monitor
		float _mfDesktopRatio;				// Desktop window's aspect ratio

		// Saves the application's root directory into _mStrRootDir on app startup
		void _setAppRootDir(void);

		// Sets logged in user's name
		void _setUsername(void);

		// Sets OS user name string
		//void _setOSname(void);

		// Checks for SSE support on hardware and OS
		//void _setSSEsupport(void);

		// Detects desktop's dimensions and stores in _miDesktopDim. Also computes aspest ratio
		void _setDesktopDims(void);

		// Detects desktop's refresh rate and stores in _miDesktopHz
		void _setDesktopHz(void);

		// Sets vendor name
		void _setVendorName(const char* szVendorName);

		// Calculates CPU time 
		void _setCPUFreq(void);

		// Stores directory path to user's app data directory (In XP this is C:\Documents and settings\USER NAME\Application data\)
		void _setUserAppDataDir(void);

		// Stores directory path to user's My Documents directory (In XP this is C:\Documents and Settings\USER NAME\My Documents\)
		void _setUserMyDocumentsDir(void);

		// Stores directory path to user's Desktop directory (In XP this is C:\Documents and Settings\USER NAME\Desktop\)
		void _setUserDesktopDir(void);
	};
